...
In ths scrum method we have inputs from customers, teams and managers to the product owner. The product owner has a product backlog, who creates a sprint planning meeting. Then there is a sprint backlog where all the information relative to the sprint is stored. The product owner represents the end-user's interests. The team builds the product, and it is cross-functional. The team is self-managing, it is autonomous and accountable. In the scrum method there is a scrum master. His role is to ensure the team's success. You can use continuous integration to maintain a single source repository. This automated the build and test.

**Week 8**

In between the two weeks there was no need to take notes. We have that refactoring is a change to the system that leaves its behavior unchanged, but enhances some nonfunctional quality for example simplicity, flexibility, understandability or the performance. Code smell is when you have unreadable code, complex structures, hard to understand, or hard to maintain and adapt. When you refactor code it could introduce new bugs or be expensive.
