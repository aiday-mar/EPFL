...
**Week 12**
We study the motivation for dense sensing and the mobility in environmental monitoring. Some examples of mobile sensor networks : pipeprobe (a mobile sensor droplet for mapping the hidden pipeline). In the OpenSense system vision we have the measurement of data and explanatory variables which together lead to high-resolution maps. These maps help to expose to information, and once again we measure information. Process-based modeling has its strengths and its weaknesses. In the overview we have a system design that leads to mobility effects, further calibration and mapping. The system performance has spatio-temporal variability. In the experimental set-up we have a traversing system with a sensor module and a smoke plume. In the mapping problem the LSDA is sparse, there is a coverage of sensors which is incomplete and dynamic. Let's consider the statistical modelling, we have the measurement of the data and the explanatory variables which lead to spatio-temporal aggregation, modelling and finally the air-pollution map.
